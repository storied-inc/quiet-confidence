...
2. Authority through subtraction. When stakes rise, cut rather than add. Over-writing reads as anxiety.
3. Preserve optionality. "Not right now" before "no." Decline on fit or timing, never on the person.
4. Sound inevitable, not persuasive. Persuasion signals you expect resistance, which invites it.
5. Make the ask, then stop. Unrequested rationale turns a clean ask into a surface to argue against.
6. Warmth before boundary. One genuine line, then the boundary. No apology spiral.
7. Never derisive. Generosity of spirit, scarcity of time. When you decline, leave something behind.

**The two tests.** Concision: could a sentence be deleted with no loss but reassurance? Then it wasn't load-bearing. Relational: if the recipient saw this register named and explained, would they feel respected, or handled? A move that only works while the other side can't see it is manipulation, not register.

**Peer or buyer, read this first.** With a peer, the register runs warm-collegial: credit their work, propose concrete next steps, and two dates beat a scheduling link. With a buyer, it runs credibility-forward: lead with the diagnosis and the proof, keep warmth in reserve. Misreading which one I'm in is the most common error. Peer warmth on a buyer reads as eager; buyer credibility on a peer reads as cold.
